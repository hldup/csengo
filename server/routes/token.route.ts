...
router.post("/delete", async (req: Request, res:Response)=> {
    ...
    if(!await Token.findOne({where:{id: req.query.id as string}})) return res.status(404).send("No token under that id") 
...
router.get("/", async (req: Request, res:Response)=> {
    const tokens = await Token.findAll({attributes: ["id","key", "createdAt"]});
